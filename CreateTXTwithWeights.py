...
def generate_txt_file_weights(output_folder, input_file, WIND_SPEED_FORECAST, 
    ...
    output_file = os.path.join(output_folder, os.path.splitext(os.path.basename(input_file))[0] + f"_final_Ws{WIND_SPEED_FORECAST}_Wd{WIND_DIRECTION_FORECAST}_w12.txt")
    txtfromcsv.write_travel_time_matrix_to_file_weights(travelMatrixToPrint, len(customers), nrLoadlevels, 12, 
...
def generate_txt_files_from_folder(seed, output_folder, input_folder, WIND_SPEED_FORECAST, 
    ...
    for root, _, files in os.walk(input_folder):
        for file in files:
            ...
            if os.path.exists(os.path.join(output_folder, os.path.splitext(os.path.basename(input_file))[0] + f"_final_Ws{WIND_SPEED_FORECAST}_Wd{np.round(WIND_DIRECTION_FORECAST,2)}_w12.txt")):
