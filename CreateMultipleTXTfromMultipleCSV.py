...
def createAVGMatrix():
    """
    AVG means average. This function uses KDE to obtain a pdf on the travel times
    for each arc in the graph of customers. The expected value of this travel time distribution
    is then outputted to a txt file that can be used in the SA-ILP-KDE algorithm.
    """
...
def generate_txt_file_weights(output_folder, input_file, WIND_SPEED_FORECAST, 
                      WIND_SPEED_SIGMA, WIND_DIRECTION_FORECAST, WIND_DIRECTION_SIGMA, weights = []):
    """
    This function creates and writes the prescribed files for all sizes in the sizes array.
    ...
    #travel_time_matrix = txtfromcsv.computeTravelTimeMatrix4D(seed, customers, distances, 
     #                       WIND_SPEED_FORECAST, WIND_SPEED_SIGMA, WIND_DIRECTION_FORECAST, WIND_DIRECTION_SIGMA, 
      #                      nrObservations, nrLoadlevels)
    travel_time_matrix, weights = txtfromcsv.computeTravelTimeMatrix4DWithWeights(customers, distances, 
                            WIND_SPEED_FORECAST, WIND_SPEED_SIGMA, WIND_DIRECTION_FORECAST, WIND_DIRECTION_SIGMA, 
                            nrLoadlevels)
    
    travelMatrixToPrint = convert4Dto2D(travel_time_matrix)
    output_file = os.path.join(output_folder, os.path.splitext(os.path.basename(input_file))[0] + f"_final_Ws{WIND_SPEED_FORECAST}_Wd{WIND_DIRECTION_FORECAST}_oWeights.txt")
    txtfromcsv.write_travel_time_matrix_to_file_weights(travelMatrixToPrint, len(customers), nrLoadlevels, 10, 
                                                output_file, WIND_SPEED_FORECAST, 
    ...
    for root, _, files in os.walk(input_folder):
        for file in files:
            ...
            generate_txt_file(seed, output_folder, input_file, WIND_SPEED_FORECAST, 
                                  WIND_SPEED_SIGMA, WIND_DIRECTION_FORECAST, WIND_DIRECTION_SIGMA)
            #generate_txt_file_weights(output_folder, input_file, WIND_SPEED_FORECAST, 
             #                     WIND_SPEED_SIGMA, WIND_DIRECTION_FORECAST, WIND_DIRECTION_SIGMA)
